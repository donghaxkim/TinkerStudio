...
describe("renderWorker", () => {
  it("renders the revision and adds an output-video artifact to it", async () => {
    ...
    const runRender: RunRender = vi.fn(async () => ({ artifacts: [
      { kind: "composition-index" as const, relativePath: "revisions/rev-1/hyperframes/index.html", url: "/u/i", mediaType: "text/html" },
      { kind: "output-video" as const, relativePath: "revisions/rev-1/hyperframes/output.mp4", url: "/u/v", mediaType: "video/mp4" },
    ] }));
