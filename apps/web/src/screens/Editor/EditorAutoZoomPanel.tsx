...
export function EditorAutoZoomPanel({ project, previewSource, onPreviewProjectChange, onAccept }: EditorAutoZoomPanelProps) {
  ...
  return (
    <section aria-label="Auto zoom suggestions" style={{ display: "grid", gap: 12, padding: 16, border: "1px solid #334155", borderRadius: 12, background: "#0f172a" }}>
      <div>
        <p style={{ margin: 0, color: "#60a5fa", fontSize: 12, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase" }}>Auto zoom</p>
        <h2 style={{ margin: "4px 0 0" }}>Cursor dwell suggestions</h2>
      </div>

      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button type="button" style={buttonStyle} onClick={suggest}>Suggest zooms</button>
        <button type="button" style={buttonStyle} disabled={status.kind !== "preview"} onClick={accept}>Accept all suggestions</button>
        <button type="button" style={buttonStyle} disabled={status.kind !== "preview"} onClick={reject}>Reject suggestions</button>
      </div>

      {status.kind === "empty" ? <p role="status" style={{ margin: 0, color: "#fde68a" }}>{status.message}</p> : null}
      ...
      {suggestions.length > 0 ? (
        <div role="status" style={{ display: "grid", gap: 8 }}>
          <strong>{suggestions.length} proposed zoom{suggestions.length === 1 ? "" : "s"}</strong>
