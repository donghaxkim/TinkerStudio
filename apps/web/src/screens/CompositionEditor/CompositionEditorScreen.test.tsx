...
describe("CompositionEditorScreen", () => {
  it("renders the standalone Playwright video shell", () => {
    render(<CompositionEditorScreen standaloneVideoUrl="/api/jobs/j/artifacts/playwright/final.mp4" repo="acme/driftboard" />);

    expect(screen.getByLabelText("Editor status")).toHaveTextContent("Saved");
    expect(screen.getByTestId("composition-standalone-video")).toHaveAttribute("src", "/api/jobs/j/artifacts/playwright/final.mp4");
