...
import { isTerminalStatus, selectArtifact, selectArtifactUrl, selectPrimaryVideoArtifact } from "./compositionGenerationClient.js";
...
describe("composition client helpers", () => {
  ...
  it("selects an artifact and its url by kind", () => {
    ...
    expect(selectArtifact(completed, "published-video")?.mediaType).toBe("video/mp4");
  ...
  it("returns undefined when the job has no result yet", () => {
    ...
    expect(selectArtifact(running, "published-video")).toBeUndefined();
    expect(selectArtifactUrl(running, "published-video")).toBeUndefined();
  });
});
