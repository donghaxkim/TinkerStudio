...
// previewed and styled without opencode/claude or the API running. Enabled by
// passing ?mock=planning in the URL (see App.tsx). Returns a ready session with
// sample messages + a valid outline immediately, and echoes a canned reply to
...
// Scripted reasoning the mock "thinks" through, revealed one line at a time so the
// planning UI can show a live agent thought-stream before the outline lands.
const THOUGHTS = [
...
export function createMockCompositionPlanningClient(): CompositionPlanningClient {
  ...
  return {
    ...
    async getSession() {
      // Reflect live thinking progress while the create request is still in flight.
      if (current !== undefined && current.status === "running") return buildThinking(DEFAULT_REQUEST);
