...
function toZoom(event: CaptureEvent, index: number, duration: number): ManualZoom[] {
...
export function compileProject(input: CompileProjectInput): DemoProject {
  ...
  const zooms: ManualZoom[] = input.captureResult.events.flatMap((event, index) =>
    toZoom(event, index, duration),
  );
