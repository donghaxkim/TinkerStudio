...
assert.match(calls[0]?.prompt ?? "", /Use website screenshots for visual fidelity/);
assert.doesNotMatch(calls[0]?.prompt ?? "", /website screenshots as fallback/);
assert.doesNotMatch(calls[0]?.prompt ?? "", /Prefer polished reconstructed UI over raw screenshots/);
