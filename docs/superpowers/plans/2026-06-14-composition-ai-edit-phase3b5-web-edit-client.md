...
- [ ] **Step 3: Implement.**
  ...
  - `CompositionEditorScreen.tsx`:
    ...
    - Clear `contextRefs` (and selection) inside `edit.accept` / `edit.reject` wrappers (wrap them: `onAccept={() => { edit.accept(); setContextRefs([]); setSelection(undefined); }}` and similarly reject).
    - Auto-replay: add an effect keyed on `edit.isPreviewing` + `model` — when `edit.isPreviewing` becomes true and `model` is ready and `lastEditedRange` is set, call `playback.playSegment(lastEditedRange.start, lastEditedRange.end)`.
